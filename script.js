...
function createRectangle() {
    const rectangle = document.createElement("textarea");
...
function startDrag(e) {
    if (findClassByString(e.target, "rectangle")) {
        ...
        document.onmousemove = function (e) {
            ...
            elementContainer.style.left =
                Math.min(Math.max(0, x), containerWidth - elementCoords.width) +
                "px";
            elementContainer.style.top =
                Math.min(
                    Math.max(0, y),
                    containerHeight - elementCoords.height
                ) + "px";
